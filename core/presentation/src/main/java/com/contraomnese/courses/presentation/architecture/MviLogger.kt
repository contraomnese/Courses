package com.contraomnese.courses.presentation.architecture

import com.contraomnese.courses.presentation.logger.LogType
import com.contraomnese.courses.presentation.logger.Logger
import com.contraomnese.courses.presentation.utils.getRandomStringNumber
...
internal class MviLogger<Action : MviAction, Effect : MviEffect, Event : MviEvent, State : MviState>(
    ...
    private fun log(tag: String, message: String, error: Throwable?) {
        if (Logger.getEnable(LogType.MVI)) {
            ...
            Logger.log(
                message = "[${Thread.currentThread().name}] $tagWithMessage",
